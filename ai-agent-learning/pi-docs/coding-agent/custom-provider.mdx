...
本页讲如何通过 `pi.registerProvider()` 把代理、自建模型、企业 OAuth/SSO 或非标准流式 API 接入 Pi。Provider registration 可以发生在 Extension factory 中；Provider config 则可以继续由 `models.json` 覆盖。核心边界是：Extension 负责注册与生命周期，`pi-ai` 负责统一 `Model`、`Context`、`AssistantMessageEventStream` 和 `Usage`，Agent Loop 不应直接解析某个厂商 response。

## 源码级中文译文

### 快速选择与注册语义

如果只需把现有 Anthropic/OpenAI/Google 请求改走 corporate proxy，调用 `pi.registerProvider("anthropic", { baseUrl, headers })` 即可，省略 `models` 会保留全部 built-in models。要注册新 Provider，指定 `models`、`baseUrl`、`api` 和认证；给出 `models` 时会替换该 Provider 原有的 model list，而不是自动合并。动态模型目录应在 async extension factory 中 fetch 和注册，这样 interactive startup 与 `pi --list-models` 都能看到它。

完整 provider（`createProvider`）适合需要自定义 authentication、过滤、refresh 或 streaming 的场景；legacy provider-config 形式适合简单配置。`unregisterProvider(name)` 会移除动态 models、key fallback、OAuth 和 custom stream handler，并恢复此前被覆盖的 built-in 行为；初始加载阶段之后的调用立即生效，不需要 `/reload`。

`api` 决定使用哪种流适配器：`anthropic-messages`、`openai-completions`、`openai-responses`、`azure-openai-responses`、`openai-codex-responses`、`mistral-conversations`、`google-generative-ai`、`google-vertex`、`bedrock-converse-stream`。大多数 OpenAI-compatible endpoint 用 `openai-completions`，但 model-specific thinking 和 provider quirks 要写入 `thinkingLevelMap`/`compat`，不能仅凭 URL 猜。

### 认证、OAuth 与 header

`apiKey` 和 `headers` 支持 `!command`、`$ENV_VAR`、`${ENV_VAR}`、`$$` 和 `$!`。`authHeader: true` 会在没有显式 `Authorization` 时添加 `Authorization: Bearer <key>`；显式 request header 优先。认证值应按请求解析并保持在受控 credential store，不要把 token 打进事件、日志、Skill 或 commit。

OAuth provider 要实现 `login(callbacks)`、`refreshToken(credentials, signal)` 和 `getApiKey(credentials)`。`OAuthLoginCallbacks` 是 UI-neutral 的边界：`onAuth` 打开 URL，`onDeviceCode` 展示 device code，`onProgress` 报告进度，`onPrompt` 读人工 token，`onSelect` 选择 browser/device flow。`OAuthCredentials` 保存 `refresh`、`access` 和毫秒级 `expires` 到 `~/.pi/agent/auth.json`；refresh 必须响应 `AbortSignal`。用户完成后以 `/login corporate-ai` 进入该流程。

### Custom streaming API 的事件契约

自定义 `streamSimple(model, context, options)` 返回 `AssistantMessageEventStream`。先建立 `AssistantMessage`，`stopReason` 初始为 `pending`，发 `start`；收到 Provider 内容时维护同一个 `partial`，按顺序发 `text_start/delta/end`、`thinking_start/delta/end` 或 `toolcall_start/delta/end`；正常完成发 `done`，异常或取消设置 `error`/`aborted` 并发 `error`。如果流结束时仍是 `pending`，应视为 Provider 协议错误，而不是成功的空答案。

文本 block 追加到 `output.content`；Tool Call 需累积 JSON delta，按 `contentIndex` 和 `toolCallId` 维护 partial，最后解析为 `{ type: "toolCall", id, name, arguments }`。Usage 从 response 更新 input/output/cache token，并调用 `calculateCost`；不要用字符数替代 Provider usage。所有 API adapter 都要处理 Unicode、空响应、断流和取消。

### Overflow、测试与安全边界

Pi 只会对它识别的 overflow 执行自动 compaction/retry。非标准 Provider 应在注册它的 Extension 中通过 `message_end` 将特定错误规范化为 `context_length_exceeded`；必须限制到自己的 `message.provider`，不能把 rate limit 或 throttling 改写成 overflow，否则会错误触发 compaction 而不是 backoff retry。改写要幂等，且一次输入的 recovery 必须有上限。

Provider 测试至少覆盖 `stream.test.ts`、tokens、abort、empty、context-overflow、image limits、Unicode surrogate、tool-call-without-result、total tokens 和 cross-provider handoff。先用 fake stream 和固定 response，断言事件顺序、partial 状态、stop reason、usage、Tool Call JSON 和 cancellation；再运行真实 model pair。失败的 provider 不应泄漏 API key，也不能把 HTTP 500、坏 JSON 或远端工具异常伪装为 final answer。

> 译者注：在 Java 中，`streamSimple` 可以映射为 `Flow.Publisher` 或回调式 `StreamingModel`，OAuth callbacks 映射为 interface，Provider config 映射为 immutable record。Go 中可用 `Model`/`Provider` interface、channel 和 `context.Context`。两种实现都要把 schema validation、usage、retry 和权限留在 Runtime 边界，而不是放进字符串 prompt。

### 可执行步骤

1. 先复制一个最接近的 `packages/ai/src/api/*.ts` adapter，列出 request、stream event 和 usage 的差异。
2. 先用 legacy config 通过 `models.json` 验证 base URL、API key、model metadata；需要 OAuth 或 custom stream 时再用 `pi.registerProvider()`。
3. 用 `ScriptedModel`/fake HTTP server 测试成功、空响应、坏 JSON、429/5xx、断流、abort 和 overflow normalization。
4. 用 `/login`、`/model`、`--list-models` 和一次受限 prompt 验证 startup、credential resolution、model visibility 和最终事件。
...
| Usage and Cost | 使用方式 and Cost |
| Context Overflow Errors | Context Overflow Errors |
| Registration | Registration |
| Testing Your Implementation | 测试 Your Implementation |
